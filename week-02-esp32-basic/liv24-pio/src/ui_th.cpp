...
lv_obj_t *scr_th         = NULL;
static lv_obj_t *lbl_th_temp = NULL;
static lv_obj_t *lbl_th_hum  = NULL;

static lv_obj_t *make_th_card(lv_obj_t *parent, const char *title,
                               const char *unit, uint32_t val_color)
{
    lv_obj_t *c = lv_obj_create(parent);
    lv_obj_set_size(c, 338, 530);
    lv_obj_set_style_bg_color(c, lv_color_hex(0x111720), 0);
    lv_obj_set_style_bg_opa(c, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(c, 20, 0);
    lv_obj_set_style_border_color(c, lv_color_hex(0x1C2E3A), 0);
    ...
    lv_obj_set_style_text_color(lbl_t, lv_color_hex(0x2A6A6A), 0);
    ...
    lv_obj_set_style_text_color(lbl_u, lv_color_hex(0x1A4A4A), 0);
...
{
    scr_th = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(scr_th, lv_color_hex(0x0A0A12), 0);
    lv_obj_set_style_bg_opa(scr_th, LV_OPA_COVER, 0);
    lv_obj_set_style_pad_all(scr_th, 0, 0);

    lv_obj_t *hdr = lv_obj_create(scr_th);
    lv_obj_set_size(hdr, 720, 72);
    lv_obj_align(hdr, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_bg_color(hdr, lv_color_hex(0x0C1215), 0);
    ...
    lv_obj_set_style_bg_color(accent, lv_color_hex(0x44CCBB), 0);
    ...
    lv_label_set_text(lbl_title, "TH-04S");
    lv_obj_set_style_text_color(lbl_title, lv_color_hex(0x44CCBB), 0);
    ...
    lv_label_set_text(lbl_sub, "CWT Temperature / Humidity");
    lv_obj_set_style_text_color(lbl_sub, lv_color_hex(0x1A4A5A), 0);
    lv_obj_set_style_text_font(lbl_sub, &lv_font_montserrat_14, 0);
    lv_obj_align(lbl_sub, LV_ALIGN_LEFT_MID, 116 + title_x, 12);

    lv_obj_t *cont = lv_obj_create(scr_th);
    ...
    lbl_th_temp = make_th_card(cont, "TEMPERATURE", "\xc2\xb0""C", 0x44FFCCu);
    lbl_th_hum  = make_th_card(cont, "HUMIDITY",    "%",           0x44BBFFu);
}
